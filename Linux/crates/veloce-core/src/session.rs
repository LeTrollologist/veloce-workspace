...
Used by both the Windows named-pipe server (`ipc_server.rs`) and the Linux
Unix socket server (`socket_server.rs`).  Each transport accept loop creates
a `ClientSession` after performing its platform-specific auth check.
*/
